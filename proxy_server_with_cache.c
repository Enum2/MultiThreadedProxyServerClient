...
{
	int remoteSocket = socket(AF_INET, SOCK_STREAM, 0);    // socket to connect to web server 
	if( remoteSocket < 0)
	{
    ...
    // actual connection making to remote server
...
bool validateRequest(ParsedRequest* request){
   return request->host && request->path && (checkHTTPversion(request->version) == 1) ;
}
...
void* thread_fn(void* socketNew)
{
	...
	else if(bytes_send_client > 0)
	{
        ...
        // the request
		int isRequestNotPared=ParsedRequest_parse(request, buffer, len) < 0;
		if (isRequestNotPared) 
		{
		...
		{	
			...
			bool isGetRequestORNot=!strcmp(request->method,"GET");
			if(isGetRequestORNot)							
			{
...
int main(int argc, char * argv[]) {
	...
	int reuse =1;
    //Allows reusing a port immediately after the program restarts
    //Preventing error of Address already in use
	if (setsockopt(proxy_socketId, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse)) < 0) 
        perror("setsockopt(SO_REUSEADDR) failed\n");

	bzero((char*)&server_addr, sizeof(server_addr));  
	server_addr.sin_family = AF_INET;
	// htons convert the port to byte order 
	...
	int i = 0; // which thread is free right now to handle request
	...
	while(1)
	{
		...
		inet_ntop( AF_INET, &ip_addr, str, INET_ADDRSTRLEN );           // converting binary ip address to foramte
